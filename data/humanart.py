...
class HumanArtDataset(Dataset):
    def __init__(
        ...
        print(f"HumanArt Dataset loaded: {len(self.data_keys)} images")
    ...
    def create_outpainting_setup(self, image):
        """
        Create outpainting setup from full image
        Returns:
            input_image: Image with center crop, rest filled with gray
            mask: 0 for center (keep), 1 for edges (generate)
        ...
        # create input image (center crop on gray background)
        input_image = Image.new('RGB', (self.image_size, self.image_size), color=(128, 128, 128))
        input_image.paste(center_crop, (self.crop_offset, self.crop_offset))

        # create mask (0 for center, 1 for outpainting region)
...
def create_dataloader(
    ...
    """
    Create DataLoader for HumanArt dataset
    ...
    Returns:
        ...
        dataset: HumanArtDataset instance
    """
    dataset = HumanArtDataset(
...
if __name__=="__main__":
    # test inference mode
    print("\n=== Inference Mode ===")
    dataset_inference = HumanArtDataset(
        image_size=512,
        center_size=256,
        return_ground_truth=False
    )
    ...
    # test training mode
    print("\n=== Training Mode ===")
    dataset_training = HumanArtDataset(
        image_size=512,
        center_size=256,
